 ...
 * Serves /user-assets/* from user-data/assets/*
 */
  ...
  return {
    ...
    async closeBundle() {
      try {
        ...
        if (!userDataPath) {
          console.log(
            "⚠️  User data path not found, skipping user-assets copy",
          );
          return;
        }
        ...
        const userAssetsDest = path.resolve(
          process.cwd(),
          "dist",
          "user-assets",
        );
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        const matchingVariant = variants.find(
          (v) => req.url && req.url.startsWith(v),
        );

        if (matchingVariant) {
          // Find files directory
          // Priority 1: Inside user-data project (e.g., user-data/paulklee/files/)
          // Priority 2: At artis root (legacy layout: /files/)
          const possibleFilesPaths = [];

          const userDataPath = getUserDataPath();
          if (userDataPath) {
            possibleFilesPaths.push(path.join(userDataPath, "files"));
          }
          possibleFilesPaths.push(
          ...
          // Thumbnails live alongside files/ (at userDataPath/thumbnails/),
          // not inside the files/ directory
          let filePath;
          if (variantName === 'thumbnails' && userDataPath) {
            const thumbPath = path.join(userDataPath, 'thumbnails', sanitizedPath);
            if (fs.existsSync(thumbPath)) {
              filePath = thumbPath;
            }
          }
          if (!filePath && filesPath) {
            filePath = path.join(filesPath, variantName, sanitizedPath);
          }
